...
> **Читать вместе с [hexlet-monolith-practices.md](hexlet-monolith-practices.md).** Первоисточник этих практик — монолит Hexlet (Rails 8.1 + Inertia 3 + Mantine 9), и там же они доведены до конца. Где реализации расходятся, брать надо монолитную: это касается как минимум флеша (§11.2 ниже) и типизации ключей переводов (§11.1).
...
У них это бесплатно: JSON-словари импортируются во фронт напрямую. У нас словари в PHP, поэтому нужен шаг выгрузки — артизан-команда, отдающая шипящиеся группы в JSON.

**Свой генератор типов при этом писать не надо.** В монолите Hexlet эта задача решена готовым `i18next-cli` с `enableSelector: true`, и ключи пишутся колбеком: `t(($) => $.admin.users.title)` — см. [hexlet-monolith-practices.md §6](hexlet-monolith-practices.md). То есть цепочка получается из двух шагов: PHP → JSON (наша команда) → типы (`i18next-cli`). `make types-check` (уже запланирован) накрывает результат без доработок.
...
Наша поправка та же, что и со `scope`: строить ключ от action-name и падать на общий ключ, если конкретного в словаре нет. Фоллбек делает переход постепенным — можно начать с двух-трёх экшенов. Место для этого — `FlashBag`, который и так пишется в PR 1.

**Реализацию брать из монолита, а не отсюда.** Там фоллбек не один захардкоженный ключ, а цепочка вверх по иерархии контроллеров: общий текст живёт на базовом контроллере, конкретный переопределяется рядом со своим экшеном — [hexlet-monolith-practices.md §2](hexlet-monolith-practices.md).
